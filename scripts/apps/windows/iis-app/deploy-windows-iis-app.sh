...
if [ -z $1 ]; then echo "Deploying Windows IIS (App) on Cluster: ${CLUSTER_ALIAS}"; else 
export CLUSTER_ALIAS="$1" && 
export CLUSTER_FULL_NAME="${CLUSTER_ALIAS}.${DOMAIN_NAME}" &&  
echo "Deploying Windows IIS (App) on Cluster: ${CLUSTER_ALIAS}"; fi

# Pause for 3 seconds
sleep 3

# Create Kubernetes objects
kubectl --context ${CLUSTER_ALIAS} create -f ./kubernetes/windows/iis-app --record

# Wait for Windows IIS (App) ELB to be created
echo "Waiting for Windows IIS (App) ELB to be created (60 seconds)"
sleep 60

# Set the DNS record prefix & the Service name and then retrieve the ELB URL
export DNS_RECORD_PREFIX="iis-app"
export SERVICE_NAME="iis-app-svc"
