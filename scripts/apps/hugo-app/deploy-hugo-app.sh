...
if [ -z $1 ]; then echo "Deploying Hugo app on Cluster: ${CLUSTER_ALIAS}"; else 
export CLUSTER_ALIAS="$1" && 
export CLUSTER_FULL_NAME="${CLUSTER_ALIAS}.${DOMAIN_NAME}" &&  
echo "Deploying Hugo app on Cluster: ${CLUSTER_ALIAS}"; fi

# Pause for 3 seconds
sleep 3

# Set Hugo App image name in the hugo-app-deploy.yaml file
sed -i -e "s|image: .*:|image: ${HUGO_APP_DOCKER_IMAGE}:|g" ./kubernetes/hugo-app/hugo-app-deploy.yaml

# Create Kubernetes objects
kubectl --context ${CLUSTER_ALIAS} create -f ./kubernetes/hugo-app --record

# Wait for Hugo App ELB to be created
echo "Waiting for Hugo App ELB to be created (60 seconds)"
sleep 60
